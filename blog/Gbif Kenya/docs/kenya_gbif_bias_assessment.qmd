...
author: "Kwizera Jean"
affiliation: "Kwiz Computing Technologies"
date: "2025-11-25"
categories: [Data Science, R Programming, Biodiversity, Data Quality, GBIF, Case Study]
image: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=800"
description: "A comprehensive analysis demonstrating reproducible workflows for biodiversity data quality assessment using GBIF data from Kenya. Features interactive visualizations and statistical methods for spatial, temporal, and taxonomic bias detection."
...
.script-modal-content {
  ...
  max-height: 85vh;
  overflow-y: auto;
  box-shadow: 0 4px 20px rgba(255, 168, 115, 0.3);
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Convert script references to clickable links
  makeScriptLinksClickable();
});

// Script information database
const scriptInfo = {
  "scripts/01_data_download.R": {
    title: "GBIF Data Download Script",
    description: "This script handles downloading biodiversity occurrence data from GBIF for Kenya.",
    sections: [
      "Line 40-60: GBIF download request setup",
      "Line 80-120: Data loading and initial filtering",
      "Line 150-180: CoordinateCleaner tests application",
      "Line 267-279: Summary statistics calculation"
    ],
    keyFunctions: ["occ_download()", "occ_download_get()", "clean_coordinates()", "summarise()"]
  },
  "scripts/01b_data_quality_assessment.R": {
    title: "Data Quality Assessment Script",
    description: "Comprehensive quality flagging and assessment of GBIF data.",
    sections: [
      "Line 90-220: Add quality flags (missing coords, high uncertainty, etc.)",
      "Line 350-380: Calculate filtering strategies (minimal, moderate, strict)",
      "Line 447-455: Compile quality assessment object"
    ],
    keyFunctions: ["add_flag_tracking()", "cc_val()", "mutate()", "bind_rows()"]
  },
  "scripts/02_spatial_bias.R": {
    title: "Spatial Bias Analysis Script",
    description: "Analyzes spatial patterns and biases in sampling effort across Kenya.",
    sections: [
      "Line 50-180: Create hexagonal grid and calculate sampling metrics",
      "Line 220-275: Moran\\'s I spatial autocorrelation test",
      "Line 320-420: Environmental bias analysis (KS tests)"
    ],
    keyFunctions: ["st_make_grid()", "Moran.I()", "ks.test()", "extract()"]
  },
  "scripts/03_temporal_bias.R": {
    title: "Temporal Bias Analysis Script",
    description: "Analyzes temporal patterns in data collection over time.",
    sections: [
      "Line 85-150: Temporal aggregation and trend analysis",
      "Line 190-250: Mann-Kendall trend tests",
      "Interactive visualization creation"
    ],
    keyFunctions: ["group_by()", "summarise()", "MannKendall()", "plot_ly()"]
  },
  "scripts/04_taxonomic_bias.R": {
    title: "Taxonomic Bias Analysis Script",
    description: "Examines taxonomic representation and diversity patterns.",
    sections: [
      "Class-level summary and Gini coefficient calculation",
      "Species rarity classification and distribution analysis",
      "Diversity indices computation"
    ],
    keyFunctions: ["count()", "ineq()", "diversity()", "specnumber()"]
  },
  "scripts/05_statistical_models.R": {
    title: "Statistical Modeling Script",
    description: "Builds GLM models to predict sampling effort and identify biases.",
    sections: [
    ...
    keyFunctions: ["glm.nb()", "predict()", "anova()", "residuals()"]
  }
};

function makeScriptLinksClickable() {
  // Find all text nodes containing script references
  ...
  while (walker.nextNode()) {
    ...
    if (node.textContent.includes("scripts/") && node.parentElement.tagName !== "SCRIPT") {
  ...
  nodesToReplace.forEach(node => {
    ...
    const scriptRegex = /(scripts\/[\\w_]+\\.R)/g;

    if (scriptRegex.test(text)) {
      const span = document.createElement("span");
      span.innerHTML = text.replace(scriptRegex, (match) => {
        return `<a class="script-link" onclick="showScriptModal(\'${match}\')">${match}</a>`;
      });
...
function showScriptModal(scriptPath) {
  const info = scriptInfo[scriptPath] || {
    title: scriptPath,
    description: "Script information not available.",
    sections: ["This is a reference script in the analysis pipeline."],
    keyFunctions: []
  };
  ...
  let content = `
    <div class="script-content">
      ...
      <ul style="color: #e8e8e8;">
        ${info.sections.map(s => `<li>${s}</li>`).join("")}
      </ul>
      ${info.keyFunctions.length > 0 ? `
        <br>
        <p><strong>Key Functions Used:</strong></p>
        <ul style="color: #e8e8e8;">
          ${info.keyFunctions.map(f => `<li><code style="color: #ffa873; background: #2a2a2a;">${f}</code></li>`).join("")}
        </ul>
      ` : ""}
      <br>
      <p style="color: #999; font-style: italic; font-size: 0.9em;">
        Note: This popup shows a summary of the script\\'s purpose and structure.
        The full analysis code is embedded inline in the document above.
      </p>
    </div>
  `;

  document.getElementById("scriptModalTitle").innerHTML = info.title;
  document.getElementById("scriptModalBody").innerHTML = content;
...
<div id="scriptModal" class="script-modal">
  <div class="script-modal-content">
    <div class="script-modal-header">
      <h2 id="scriptModalTitle">Script Information</h2>
      <span class="script-modal-close" onclick="closeScriptModal()">&times;</span>
    </div>
    <div id="scriptModalBody"></div>
  </div>
</div>
')
```
...
# Note: utils.R file not found - functions defined inline if needed
# source(here("blog", "Gbif Kenya", "R", "utils.R"))

# Set paths - relative to blog/Gbif Kenya directory
data_processed <- here("blog", "Gbif Kenya", "data", "processed")
data_outputs <- here("blog", "Gbif Kenya", "data", "outputs")
figures_dir <- here("blog", "Gbif Kenya", "figures")
...
required_files <- c(
  here("blog", "Gbif Kenya", "results", "data_quality", "data_quality_assessment.rds"),
  here("blog", "Gbif Kenya", "results", "data_quality", "kenya_gbif_flagged.rds"),
  here("blog", "Gbif Kenya", "results", "spatial_bias", "spatial_bias_summary.rds"),
  here("blog", "Gbif Kenya", "results", "temporal_bias", "temporal_bias_summary.rds"),
  here("blog", "Gbif Kenya", "results", "taxonomic_bias", "taxonomic_bias_summary.rds")
)
...
quality_assessment <- readRDS(here("blog", "Gbif Kenya", "results", "data_quality", "data_quality_assessment.rds"))
...
kenya_flagged <- readRDS(here("blog", "Gbif Kenya", "results", "data_quality", "kenya_gbif_flagged.rds"))

spatial_summary <- readRDS(here("blog", "Gbif Kenya", "results", "spatial_bias", "spatial_bias_summary.rds"))
moran_results <- spatial_summary$moran_results

temporal_summary <- readRDS(here("blog", "Gbif Kenya", "results", "temporal_bias", "temporal_bias_summary.rds"))
temporal_stats <- temporal_summary$temporal_stats

taxonomic_summary <- readRDS(here("blog", "Gbif Kenya", "results", "taxonomic_bias", "taxonomic_bias_summary.rds"))
...
metadata <- readRDS(here("blog", "Gbif Kenya", "data", "processed", "metadata.rds"))
summary_stats <- readRDS(here("blog", "Gbif Kenya", "data", "processed", "summary_stats.rds"))
```
...
```{r load-spatial-data}
spatial_grid <- readRDS(here("blog", "Gbif Kenya", "results", "spatial_bias", "spatial_grid_effort.rds"))
moran_results <- spatial_summary$moran_results
...
temporal_bias_summary <- readRDS(here("blog", "Gbif Kenya", "results", "temporal_bias","temporal_bias_summary.rds"))
...
taxonomic_summary <- readRDS(here("blog", "Gbif Kenya", "results", "taxonomic_bias", "taxonomic_bias_summary.rds"))
class_summary <- readRDS(here("blog", "Gbif Kenya", "results", "taxonomic_bias", "class_summary.rds"))
rarity_summary <- readRDS(here("blog", "Gbif Kenya", "results", "taxonomic_bias", "rarity_summary.rds"))
```
...
```{r load-model-data}
modeling_summary <- readRDS(here("blog", "Gbif Kenya", "results", "statistical_models",  "modeling_summary.rds"))
model_summaries <- readRDS(here("blog", "Gbif Kenya", "results", "statistical_models",  "model_summaries.rds"))
```
...
knitr::include_graphics(file.path(figures_dir, "data_quality_filter_levels.png"))
```
...
spatial_grid_file <- here("blog", "Gbif Kenya", "results", "spatial_bias", "spatial_grid_effort.rds")
...
leaflet(spatial_grid_wgs84) %>%
  ...
  addPolygons(
    ...
    popup = ~{
      ...
      paste0(
        "<div style='color: #ffa873; font-family: Arial, sans-serif;'>",
        "<strong style='color: #ffa873;'>Grid Cell Summary</strong><br>",
        "<strong style='color: #ffa873;'>Longitude:</strong> ", round(coords[1], 4), "°<br>",
        "<strong style='color: #ffa873;'>Latitude:</strong> ", round(coords[2], 4), "°<br>",
        "<strong style='color: #ffa873;'>Location:</strong> Grid Cell (", round(coords[1], 2), "°, ", round(coords[2], 2), "°)<br>",
        "<hr style='border-color: #ffa873;'>",
        "<strong style='color: #ffa873;'>Records:</strong> ", format(n_records, big.mark = ","), "<br>",
        "<strong style='color: #ffa873;'>Species:</strong> ", n_species, "<br>",
        "<strong style='color: #ffa873;'>Log10 Records:</strong> ", round(log10(n_records + 1), 2),
        "</div>"
      )
    },
...
env_comp_file <- here("blog", "Gbif Kenya", "results", "spatial_bias",  "environmental_comparison.rds")
...
fig_treemap <- plot_ly(
  ...
  layout(
    title = list(
      text = "Taxonomic Representation in Gbif KenyaData (Top 15 Classes)",
      font = list(size = 16)
    ),
  ...
  leaflet(grid_wgs84) %>%
    ...
    addPolygons(
      ...
      popup = ~{
        ...
        paste0(
          "<div style='color: #ffa873; font-family: Arial, sans-serif;'>",
          "<strong style='color: #ffa873;'>Sampling Bias Analysis</strong><br>",
          "<strong style='color: #ffa873;'>Longitude:</strong> ", round(coords[1], 4), "°<br>",
          "<strong style='color: #ffa873;'>Latitude:</strong> ", round(coords[2], 4), "°<br>",
          "<strong style='color: #ffa873;'>Location:</strong> Grid Cell (", round(coords[1], 2), "°, ", round(coords[2], 2), "°)<br>",
          "<hr style='border-color: #ffa873;'>",
          "<strong style='color: #ffa873;'>Observed:</strong> ", format(n_records, big.mark = ","), " records<br>",
          "<strong style='color: #ffa873;'>Predicted:</strong> ", format(round(predicted_count), big.mark = ","), " records<br>",
          "<strong style='color: #ffa873;'>Residual:</strong> ", round(residual_count, 1), "<br>",
          "<strong style='color: #ffa873;'>Std. Residual:</strong> ", round(residual_std, 2), "<br>",
          "<hr style='border-color: #ffa873;'>",
          ifelse(residual_std < -1, "<strong style='color: #2166AC;'>Under-sampled</strong>",
                 ifelse(residual_std > 1, "<strong style='color: #B2182B;'>Over-sampled</strong>",
                        "<strong style='color: #ffa873;'>As expected</strong>")),
          "</div>"
        )
      },
      highlightOptions = highlightOptions(
  ...
  map_filtered <- leaflet(grid_filtered_wgs84) %>%
    ...
    addPolygons(
      ...
      popup = ~{
        ...
        paste0(
          "<div style='color: #ffa873; font-family: Arial, sans-serif;'>",
          "<strong style='color: #ffa873;'>Filtered Sampling Bias</strong><br>",
          "<strong style='color: #ffa873;'>Longitude:</strong> ", round(coords[1], 4), "°<br>",
          "<strong style='color: #ffa873;'>Latitude:</strong> ", round(coords[2], 4), "°<br>",
          "<strong style='color: #ffa873;'>Location:</strong> Grid Cell (", round(coords[1], 2), "°, ", round(coords[2], 2), "°)<br>",
          "<hr style='border-color: #ffa873;'>",
          "<strong style='color: #ffa873;'>Observed:</strong> ", format(n_records, big.mark = ","), " records<br>",
          "<strong style='color: #ffa873;'>Predicted:</strong> ", format(round(predicted_count), big.mark = ","), " records<br>",
          "<strong style='color: #ffa873;'>Residual:</strong> ", round(residual_filtered, 1), "<br>",
          "<strong style='color: #ffa873;'>Std. Residual (filtered):</strong> ", round(residual_std_filtered, 2), "<br>",
          "<hr style='border-color: #ffa873;'>",
          ifelse(residual_std_filtered < -1, "<strong style='color: #2166AC;'>Under-sampled</strong>",
                 ifelse(residual_std_filtered > 1, "<strong style='color: #B2182B;'>Over-sampled</strong>",
                        "<strong style='color: #ffa873;'>As expected</strong>")),
          "</div>"
        )
      },
    ...
    addPolygons(
      ...
      popup = ~{
        ...
        paste0(
          ...
          "<strong style='color: #ffa873;'>Species:</strong> ", n_species, "<br>",
          "<strong style='color: #ffa873;'>Records:</strong> ", format(n_records, big.mark = ","), "<br>",
          "<strong style='color: #ffa873;'>Log10 Species:</strong> ", round(log10(n_species + 1), 2),
          "</div>"
        )
      },
      highlightOptions = highlightOptions(
